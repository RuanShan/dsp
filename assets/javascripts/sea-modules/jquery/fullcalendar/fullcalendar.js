...
define(function(require, exports, moudles) {
    var jQuery = require("jquery"),moment=require("moment");
    !(function($, moment) {
        var fc = $.fullCalendar = {
            version: "2.4.0"
        };
        ...
        */
        ...
        var Grid = fc.Grid = RowRenderer.extend({
            ...
            bgCellHtml: function(cell) {
                ...
                var exits ="";
                var exitsclass ="";
                if(view.options.outDataSource){
                    var index = view.options.outDataSource.map(function(item) {
                        return item.date;
                    }).indexOf(getdate);
                    var data=view.options.outDataSource[index];
                    if (data) {
                        title = data.title;
                        exits = data.exits;
                        if(title){
                            title="<span class='outer-title-data'>"+title+"</span>";
                        }
                        if(exits){
                            exitsclass="disabled";
                            title="";
                            if(title){
                                exits="<span class='outer-exits-datas'>"+exits+"</span>";
                            }else{
                                exits="<span class='outer-exits-data'>"+exits+"</span>";
                            }
                ...
                return '<td class="'+exitsclass+" " + classes.join(' ') + '"' +
        ...
        var View = fc.View = Class.extend({
            ...
            displayView: function(date) {
                ...
                var _this=this;
                if(this.options.loadOutDate){
                   $.ajax(this.options.loadOutDate, {
                        type: "post",
                        data: {
                        ...
                        _this.options.outDataSource=d;
                        _this.renderDates();
                        _this.updateSize();
                        _this.renderBusinessHours();
                    }).fail(function() {
                        _this.options.outDataSource=[];
                        _this.renderDates();
                        _this.updateSize();
                        _this.renderBusinessHours();
                    }); 
