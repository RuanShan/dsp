define(function(require, exports, module) {
	...
	$.fn.slide=function(options){
		$.fn.slide.defaults={
			...
			targetCell:null,				 //切换元素对象，处理内容切换元素非包裹状态下等情况，实现更多效果，处理更多情况，能和titCell、mainCell同时使用。只支持fade,和slideDown效果。
			...
			titOnClassName:"on",             //当前titCell位置自动选中时的class名称
			autoPage:false, 				 // 程序自动分页，需结合titCell使用，若为true，则titCell为导航元素的包裹层对象。ps：scroll>1时，记得设置autoPage:true，否则分页错误。 支持自定义分页html，$为数字替换位置。例如autoPage:"<li><a>$</a></li>"，如果不想输出数字：autoPage:"<li><a></a></li>"。
			prevCell:".prev",                //前一个/页按钮对象。
			nextCell:".next",                //后一个/页按钮对象。
			pageStateCell:".pageState",      //分页状态对象，用于显示分页状态，例如：2/3
			opp: false,                      //默认反方向运动，例如 effect:"leftMarquee" 然后设置 opp:true，则效果为右滚动。常用于leftMarquee/topMarquee
			pnLoop:true, 					 //前/后按钮是否继续循环，若为false则当翻动到最前/后页时，前/后按钮点击无效，同时增加prevStop/nextStop类名控制样色
			easing:"swing",                  //缓动效果；更改默认效果为“swing”，使效果更流畅
			startFun:null,                   //每次切换效果开始时执行函数，用于处理特殊情况或创建更多效果
			endFun:null,					 //每次切换效果结束时执行函数，用法和startFun一致
			switchLoad:null,                 //内容切换加载，暂时只支持图片，必须配合后台程序使用。当为图片的时候switchLoad为图片实际路径的属性名称，例如switchLoad:"_src"
		...
		return this.each(function() {
			...
			//开始回调
			var doStartFun=function(){ if ( $.isFunction( opts.startFun) ){ opts.startFun({
					index : index,
					navObjSize : navObjSize,
					slider : slider,
					titCell : $(opts.titCell, slider),
					conBox : conBox,
					tarObj : tarObj,
					prevBtn : prevBtn,
					nextBtn : nextBtn
			...
			//结束回调
			var doEndFun=function(){ if ( $.isFunction( opts.endFun ) ){ opts.endFun({
					index : index,
					navObjSize : navObjSize,
					slider : slider,
					titCell : $(opts.titCell, slider),
